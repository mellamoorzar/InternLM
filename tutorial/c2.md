...
&emsp;&emsp;本课的内容是书生·浦语大模型趣味 Demo。包含大模型的介绍和，以及三个不同的大模型 Demo（分别是 InternLM-Chat-7B 智能对话 Demo、Lagent 智能体工具调用 Demo 和浦语·灵笔图文创作理解 Demo）。通过演示和实践，展示了大模型的训练和应用的过程，以及如何使用相关工具进行模型下载和通用环境配置。

## 1 InternLM-Chat-7B 智能对话 Demo

使用 [InternStudio](https://studio.intern-ai.org.cn/) 中的 A100(1/4) 机器和 `InternLM-Chat-7B` 模型部署一个智能对话 Demo。

### 1.1 环境准备

在 [InternStudio](https://studio.intern-ai.org.cn/) 平台中选择 A100(1/4) 的配置，镜像选择 `Cuda11.7-conda`。

打开刚刚租用服务器的`进入开发机`，并且打开其中的终端开始环境配置、模型下载和运行 `demo`。

进入开发机后，在页面的左上角可以切换 `JupyterLab`、`终端`和 `VScode`，并在终端输入 `bash` 命令，进入 `conda` 环境。
...
[InternStudio](https://studio.intern-ai.org.cn/) 平台的 `share` 目录下已经有全系列的 `InternLM` 模型，直接复制即可。使用如下命令复制：
...
在 `/root` 路径下新建目录 `model`，在目录下新建 `download.py` 文件并在其中输入以下内容，粘贴代码后记得保存文件。并运行 `python /root/model/download.py` 执行下载，模型大小为 14GB，下载模型大概需要 10-20 分钟。
...
使用 [InternStudio](https://studio.intern-ai.org.cn/) 中的 A100(1/4) 机器、`InternLM-Chat-7B` 模型和 `Lagent` 框架部署一个智能工具调用 Demo。
...
用同样的方法，依然切换到 `VScode` 页面，运行成功后，将端口映射到本地。在本地浏览器输入 `http://127.0.0.1:6006` 即可。
...
> 这里 `num_gpus 1` 是因为InternStudio平台对于 `A100(1/4)*2` 识别仍为一张显卡。但如果使用两张 3090 来运行此 demo，仍需将 `num_gpus` 设置为 `2` 。

将端口映射到本地。在本地浏览器输入 `http://127.0.0.1:6006` 即可。以`又见敦煌`为提示词，体验图文创作的功能。
...
更多详细内容可参考 [MirrorZ Help](https://help.mirrors.cernet.edu.cn/)。

#### 4.1.1 pip 换源

临时使用镜像源安装，如下所示：`some-package` 为需要安装的包名。
...
由于服务器通常只暴露了用于安全远程登录的 SSH（Secure Shell）端口，如果需要访问服务器上运行的其他服务（如 web 应用）的特定端口，需要一种特殊的设置。可以通过使用SSH隧道的方法，将服务器上的这些特定端口映射到本地计算机的端口。这样做的步骤如下：

首先配置本地的 `SSH Key` ，以 `Windows` 为例。
...
步骤②：被提示选择密钥文件的保存位置，默认情况下是在 `~/.ssh/` 目录中。按 `Enter` 键接受默认值或输入自定义路径。
...
***以下下载模型的操作会占用开发机的大量带宽和内存，下载等待的时间也会比较长。可以在本地电脑尝试***
